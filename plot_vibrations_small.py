...
#
# NOTE: This is meant to handle identical brushless motor models, but different motors on the same copter frame.  The
# plot_vibrations.py script is more meant to do a comparison of two different motor models to see if one model is better
# than another.  This is more meant to compare the same motor model, but different serial numbers, within a given copter
# implementation.
#   
...
def parse_args():
    parser = ArgumentParser(description='Generate spectral density (vibration) plots from pyulog data files.')
    parser.add_argument('-o', '--output', dest='output', help='Output file for plots (HTML format).  If unspecified, named automatically based on model and motor fields.')
    parser.add_argument('-m', '--motor', action='append', default=[], required=True, help='Name of the motor subdirectory logfiles to generate comparison vibration plots.')
    parser.add_argument('--model', action='store', default='sunnysky_X2212-13_980kv', help='Name of motor model.  The format of subdirectories containing the logs should be <model>_<motor>')
...
def main():
    ...
    parsed = parse_args()
    regexp = parsed.model + '_('
    for motor in parsed.motor:
        regexp = regexp + '(' + motor + ')|'
    regexp = regexp.rstrip('|') + ')' #Remove the last '|' before adding final grouping
    re_comp = re.compile(regexp)
    if not parsed.output:
        output = 'plots_' + parsed.model + '_' + '_'.join(parsed.motor) + '.html'
    else:
        output = parsed.output
    output_file(output)
    ulogfiles = list(filter(lambda s: re_comp.match(s), map(lambda s: str(s), Path('./').glob('**/*.ulg'))))
    #Determine the number of rpm classes - should be consistent b/w all motors
    re_motor = re.compile(parsed.model + '_' + parsed.motor[0])
    ulogfiles_motor = list(filter(lambda ul: re_motor.match(ul), ulogfiles))
    num_rpm_classes = len(ulogfiles_motor)
    plots_grid = [None] * num_rpm_classes  #Rows are the rpm class, columns are motor identifiers
    for r in range(len(plots_grid)):
        plots_grid[r] = [None] * len(parsed.motor) #Number of columns equal the motor identifier  
    msg_filter = ['sensor_combined']
    #Organize/sort files by ordering in parsed.motors, and then write/add subplots of each motor designation to a given column
    for column,motor in enumerate(parsed.motor):
        re_motor = re.compile(parsed.model + '_' + parsed.motor[column])
        ulogfiles_motor = list(filter(lambda ul: re_motor.match(ul), ulogfiles))
        ulogfiles_motor.sort() #Just to make sure we sort in order of pwm values -- so that everything is in the same order row-wise when doing visual comparisons
        for row,ulogfile in enumerate(ulogfiles_motor):
            ...
            data_plot = DataPlotSpec(data, plot_config, 'sensor_combined',
                                    y_axis_label='[Hz]', title='{} (PWM {}) Acceleration Power Spectral Density'.format(motor, pwm),
            ...
            if data_plot.finalize() is not None: 
                print("{}, {}".format(pwm,parsed.motor[column]))
                plots_grid[row][column] = data_plot._p

    #Output plots as a gridplot
    g = gridplot(plots_grid, merge_tools=True, sizing_mode="scale_width")
    show(g)
